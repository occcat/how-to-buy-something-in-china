...
- `env-check/`：阶段 2，检查 macOS、ego lite、登录态缓存、中文 LaTeX PDF 导出能力和 Sub Agent 授权。
...
- `report-export/`：阶段 5，生成固定四章、四张选型表格、最终 Top 3 的 Markdown 报告和详细 PDF；有可正常处理中文的 LaTeX 引擎时优先使用 LaTeX→PDF，不可用时降级为 HTML→PDF，报告大纲和内容架构不变。
...
- PDF 导出用的 LaTeX 源文件与编译辅助文件，或降级链路使用的 HTML/CSS/资源文件，只能放在本任务缓存目录；PDF 生成并验证后必须删除，不得作为最终产物保留。
- `report/` 只放最终 Markdown/PDF 报告，必须被 Git 忽略；同一日期下按 `report/YYYY-MM-DD/{md,pdf}/` 拆分产物类型。
- 调用浏览器自动化写临时 Node.js 脚本时，只能写入本任务缓存目录，结束后删除。
- 不在根目录遗留临时抓取文件、截图、LaTeX/HTML 转换文件或调试脚本。
